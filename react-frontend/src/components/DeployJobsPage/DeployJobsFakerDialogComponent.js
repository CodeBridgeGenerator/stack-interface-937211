...
import deployJobsFakerFactory from "./deployJobsFakerFactory";
...
const DeployJobsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = deployJobsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("deployJobs").create(elem));
...
export default connect(mapState, mapDispatch)(DeployJobsFakerDialogComponent);
