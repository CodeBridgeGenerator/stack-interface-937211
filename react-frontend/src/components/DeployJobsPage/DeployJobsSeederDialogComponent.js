...
import seederData from "./deployJobs.seeder.json";
...
const DeployJobsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("deployJobs").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(DeployJobsSeederDialogComponent);
