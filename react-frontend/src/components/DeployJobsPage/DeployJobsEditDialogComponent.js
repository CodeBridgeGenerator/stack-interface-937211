...
port: _entity?.port,
        ...
        const result = await client.service("deployJobs").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info deployJobs updated successfully" });
    ...
    return (
        <Dialog header="Edit DeployJobs" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="deployJobs-edit-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            ...
            <Calendar id="start" value={_entity?.start ? new Date(_entity?.start) : new Date()} onChange={ (e) => setValByKey("start", new Date(e.target.value))} showIcon showButtonBar  />
        </span>
        </div>
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
