...
const DeployJobsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            projectId: _entity?.projectId,start: _entity?.start,status: _entity?.status || false,url: _entity?.url,port: _entity?.port,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info DeployJobs created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in DeployJobs" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create DeployJobs" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="deployJobs-create-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <Calendar id="start"  value={_entity?.start ? new Date(_entity?.start) : new Date()} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("start", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputNumber id="port" className="w-full mb-3 p-inputtext-sm" value={_entity?.port} onChange={(e) => setValByKey("port", e.target.value)}  />
            </span>
            <small className="p-error">
