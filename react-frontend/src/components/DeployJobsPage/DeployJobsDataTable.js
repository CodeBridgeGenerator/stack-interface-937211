...
const DeployJobsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_calendarTemplate1 = (rowData, { rowIndex }) => <p >{rowData.start}</p>
const p_booleanTemplate2 = (rowData, { rowIndex }) => <p >{String(rowData.status)}</p>
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.url}</p>
const p_numberTemplate4 = (rowData, { rowIndex }) => <p >{rowData.port}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "deployJobs"});
...
<Column field="start" header="Start" body={p_calendarTemplate1} filter={selectedFilterFields.includes("start")} hidden={selectedHideFields?.includes("start")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="url" header="Url" body={pTemplate3} filter={selectedFilterFields.includes("url")} hidden={selectedHideFields?.includes("url")}  sortable style={{ minWidth: "8rem" }} />
<Column field="port" header="Port" body={p_numberTemplate4} filter={selectedFilterFields.includes("port")} hidden={selectedHideFields?.includes("port")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload DeployJobs Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search DeployJobs" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default DeployJobsDataTable;