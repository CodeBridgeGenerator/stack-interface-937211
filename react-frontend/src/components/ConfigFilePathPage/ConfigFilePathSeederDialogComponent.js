...
import seederData from "./configFilePath.seeder.json";
...
const ConfigFilePathSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("configFilePath").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ConfigFilePathSeederDialogComponent);
