...
const ConfigFilePathDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.ConfigFilePath}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "configFilePath"});
...
<Column field="ConfigFilePath" header="ConfigFilePath" body={pTemplate0} filter={selectedFilterFields.includes("ConfigFilePath")} hidden={selectedHideFields?.includes("ConfigFilePath")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload ConfigFilePath Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search ConfigFilePath" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ConfigFilePathDataTable;