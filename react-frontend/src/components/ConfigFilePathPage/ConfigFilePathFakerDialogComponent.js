...
import configFilePathFakerFactory from "./configFilePathFakerFactory";
...
const ConfigFilePathFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = configFilePathFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("configFilePath").create(elem));
...
export default connect(mapState, mapDispatch)(ConfigFilePathFakerDialogComponent);
