...
const ServiceGenTempDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.schemaGen?.appName}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.serviceTemp}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "serviceGenTemp"});
...
<Column field="schemaGen" header="SchemaGen" body={dropdownTemplate0} filter={selectedFilterFields.includes("schemaGen")} hidden={selectedHideFields?.includes("schemaGen")}  style={{ minWidth: "8rem" }} />
<Column field="serviceTemp" header="ServiceTemp" body={pTemplate1} filter={selectedFilterFields.includes("serviceTemp")} hidden={selectedHideFields?.includes("serviceTemp")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload ServiceGenTemp Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search ServiceGenTemp" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ServiceGenTempDataTable;