...
import seederData from "./serviceGenTemp.seeder.json";
...
const ServiceGenTempSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("serviceGenTemp").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ServiceGenTempSeederDialogComponent);
