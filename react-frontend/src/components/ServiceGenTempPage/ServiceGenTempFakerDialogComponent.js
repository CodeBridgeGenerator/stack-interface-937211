...
import serviceGenTempFakerFactory from "./serviceGenTempFakerFactory";
...
const ServiceGenTempFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [schemaGenIds, setschemaGenIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("stackName")
                        .find({ query: { $select: ["_id","appName"] } })
                        .then((data) => {
                        setschemaGenIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = serviceGenTempFakerFactory(props.user,fakerCount,schemaGenIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("serviceGenTemp").create(elem));
...
export default connect(mapState, mapDispatch)(ServiceGenTempFakerDialogComponent);
