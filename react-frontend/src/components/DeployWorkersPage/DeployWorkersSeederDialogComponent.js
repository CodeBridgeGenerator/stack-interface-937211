...
import seederData from "./deployWorkers.seeder.json";
...
const DeployWorkersSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("deployWorkers").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(DeployWorkersSeederDialogComponent);
