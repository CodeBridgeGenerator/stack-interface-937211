...
const DeployWorkersDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.deployerJobId?.url}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.process}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.task}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "deployWorkers"});
...
<Column field="deployerJobId" header="DeployerJobId" body={dropdownTemplate0} filter={selectedFilterFields.includes("deployerJobId")} hidden={selectedHideFields?.includes("deployerJobId")}  style={{ minWidth: "8rem" }} />
<Column field="process" header="Process" body={pTemplate1} filter={selectedFilterFields.includes("process")} hidden={selectedHideFields?.includes("process")}  sortable style={{ minWidth: "8rem" }} />
<Column field="task" header="Task" body={pTemplate2} filter={selectedFilterFields.includes("task")} hidden={selectedHideFields?.includes("task")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload DeployWorkers Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search DeployWorkers" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default DeployWorkersDataTable;