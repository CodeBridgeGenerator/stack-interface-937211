...
import deployWorkersFakerFactory from "./deployWorkersFakerFactory";
...
const DeployWorkersFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [deployerJobIdIds, setdeployerJobIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("deployJobs")
                        .find({ query: { $select: ["_id","url"] } })
                        .then((data) => {
                        setdeployerJobIdIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = deployWorkersFakerFactory(props.user,fakerCount,deployerJobIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("deployWorkers").create(elem));
...
export default connect(mapState, mapDispatch)(DeployWorkersFakerDialogComponent);
