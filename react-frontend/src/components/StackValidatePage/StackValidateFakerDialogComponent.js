...
import stackValidateFakerFactory from "./stackValidateFakerFactory";
...
const StackValidateFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("stackschema")
                        .find({ query: { $select: ["_id","rootPath"] } })
                        .then((data) => {
    ...
    const onRun = async () => {
        let fakeData = stackValidateFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stackValidate").create(elem));
...
export default connect(mapState, mapDispatch)(StackValidateFakerDialogComponent);
