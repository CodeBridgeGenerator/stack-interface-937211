...
import seederData from "./stackValidate.seeder.json";
...
const StackValidateSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("stackValidate").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(StackValidateSeederDialogComponent);
