...
const StackValidateDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.name?.rootPath}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.stackValidate}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "stackValidate"});
...
<Column field="stackValidate" header="StackValidate" body={pTemplate1} filter={selectedFilterFields.includes("stackValidate")} hidden={selectedHideFields?.includes("stackValidate")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload StackValidate Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search StackValidate" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default StackValidateDataTable;