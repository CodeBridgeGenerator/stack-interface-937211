...
const StackValidateCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.stackValidate)) {
                error["stackValidate"] = `StackValidate field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name?._id,stackValidate: _entity?.stackValidate,
        ...
        const result = await client.service("stackValidate").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info StackValidate updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in StackValidate" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create StackValidate" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="stackValidate-create-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
            ...
            <small className="p-error">
            {!_.isEmpty(error["stackValidate"]) ? (
              <p className="m-0" key="error-stackValidate">
                {error["stackValidate"]}
              </p>
            ) : null}
          </small>
