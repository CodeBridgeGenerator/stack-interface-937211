...
import seederData from "./pathToLogo.seeder.json";
...
const PathToLogoSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("pathToLogo").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(PathToLogoSeederDialogComponent);
