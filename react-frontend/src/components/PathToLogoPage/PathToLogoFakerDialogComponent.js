...
import pathToLogoFakerFactory from "./pathToLogoFakerFactory";
...
const PathToLogoFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("appvalidate")
                        .find({ query: { $select: ["_id","AppName"] } })
                        .then((data) => {
                        setPathIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = pathToLogoFakerFactory(props.user,fakerCount,PathIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("pathToLogo").create(elem));
...
export default connect(mapState, mapDispatch)(PathToLogoFakerDialogComponent);
