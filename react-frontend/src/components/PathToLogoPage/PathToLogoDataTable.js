...
const PathToLogoDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.Path?.AppName}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "pathToLogo"});
...
<Column field="Path" header="Path" body={dropdownTemplate0} filter={selectedFilterFields.includes("Path")} hidden={selectedHideFields?.includes("Path")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload PathToLogo Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search PathToLogo" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default PathToLogoDataTable;