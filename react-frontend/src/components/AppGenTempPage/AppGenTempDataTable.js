...
const AppGenTempDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.AppStackName?.appName}</p>
const dropdownArrayTemplate1 = (rowData, { rowIndex }) => <p >{rowData.apptemp}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "appGenTemp"});
...
<Column field="AppStackName" header="AppStackName" body={dropdownTemplate0} filter={selectedFilterFields.includes("AppStackName")} hidden={selectedHideFields?.includes("AppStackName")}  style={{ minWidth: "8rem" }} />
<Column field="apptemp" header="Apptemp" body={dropdownArrayTemplate1} filter={selectedFilterFields.includes("apptemp")} hidden={selectedHideFields?.includes("apptemp")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AppGenTemp Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AppGenTemp" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AppGenTempDataTable;