...
import AppGenTempDatatable from "./AppGenTempDataTable";
...
import AppGenTempFakerDialogComponent from "./AppGenTempFakerDialogComponent";
import AppGenTempSeederDialogComponent from "./AppGenTempSeederDialogComponent";

const AppGenTempPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("appGenTemp")
            .find({ query: { $limit: 10000 , AppStackName : urlParams.singleStackNameId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
    ...
    const service = _.find(config.services, { serviceName : "appGenTemp"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("appGenTemp").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "AppGenTemp", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("appGenTemp").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">AppGenTemp </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="appGenTemp-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="appGenTemp-datatable">
                    <AppGenTempDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <AppGenTempEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AppGenTempCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <AppGenTempFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AppGenTempSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AppGenTempPage);
