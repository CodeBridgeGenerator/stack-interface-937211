...
import seederData from "./appGenTemp.seeder.json";
...
const AppGenTempSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("appGenTemp").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AppGenTempSeederDialogComponent);
