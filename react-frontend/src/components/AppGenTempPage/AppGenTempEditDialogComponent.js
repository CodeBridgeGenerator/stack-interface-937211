...
const AppGenTempCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            AppStackName: _entity?.AppStackName?._id,
apptemp: _entity?.apptemp,
        };

        setLoading(true);
        try {
            
        await client.service("appGenTemp").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info appGenTemp updated successfully" });
    ...
    return (
        <Dialog header="Edit AppGenTemp" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="appGenTemp-edit-dialog-component">
