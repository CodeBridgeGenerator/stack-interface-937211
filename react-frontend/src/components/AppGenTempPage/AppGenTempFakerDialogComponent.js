...
import appGenTempFakerFactory from "./appGenTempFakerFactory";
...
const AppGenTempFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [AppStackNameIds, setAppStackNameIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("stackName")
                        ...
                        .then((data) => {
                        setAppStackNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = appGenTempFakerFactory(props.user,fakerCount,AppStackNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("appGenTemp").create(elem));
...
export default connect(mapState, mapDispatch)(AppGenTempFakerDialogComponent);
