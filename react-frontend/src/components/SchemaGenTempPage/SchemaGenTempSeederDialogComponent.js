...
import seederData from "./schemaGenTemp.seeder.json";
...
const SchemaGenTempSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("schemaGenTemp").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(SchemaGenTempSeederDialogComponent);
