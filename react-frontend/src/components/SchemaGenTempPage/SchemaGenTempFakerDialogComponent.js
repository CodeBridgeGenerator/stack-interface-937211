...
import schemaGenTempFakerFactory from "./schemaGenTempFakerFactory";
...
const SchemaGenTempFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = schemaGenTempFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("schemaGenTemp").create(elem));
...
export default connect(mapState, mapDispatch)(SchemaGenTempFakerDialogComponent);
