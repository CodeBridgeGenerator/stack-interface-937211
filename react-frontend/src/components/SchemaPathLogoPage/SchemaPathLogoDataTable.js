...
const SchemaPathLogoDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.Logo?.name}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.pathToLogo}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "schemaPathLogo"});
...
<Column field="Logo" header="Logo" body={dropdownTemplate0} filter={selectedFilterFields.includes("Logo")} hidden={selectedHideFields?.includes("Logo")}  style={{ minWidth: "8rem" }} />
<Column field="pathToLogo" header="PathToLogo" body={pTemplate1} filter={selectedFilterFields.includes("pathToLogo")} hidden={selectedHideFields?.includes("pathToLogo")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload SchemaPathLogo Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search SchemaPathLogo" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default SchemaPathLogoDataTable;