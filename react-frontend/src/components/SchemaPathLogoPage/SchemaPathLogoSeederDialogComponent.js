...
import seederData from "./schemaPathLogo.seeder.json";
...
const SchemaPathLogoSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("schemaPathLogo").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(SchemaPathLogoSeederDialogComponent);
