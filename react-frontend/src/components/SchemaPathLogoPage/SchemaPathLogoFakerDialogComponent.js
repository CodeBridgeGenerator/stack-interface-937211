...
import schemaPathLogoFakerFactory from "./schemaPathLogoFakerFactory";
...
const SchemaPathLogoFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("schema")
                        ...
                        .then((data) => {
                        setLogoIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = schemaPathLogoFakerFactory(props.user,fakerCount,LogoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("schemaPathLogo").create(elem));
...
export default connect(mapState, mapDispatch)(SchemaPathLogoFakerDialogComponent);
