...
const SchemaPathLogoCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            Logo: _entity?.Logo?._id,
pathToLogo: _entity?.pathToLogo,
        };

        setLoading(true);
        try {
            
        await client.service("schemaPathLogo").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info schemaPathLogo updated successfully" });
    ...
    return (
        <Dialog header="Edit SchemaPathLogo" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="schemaPathLogo-edit-dialog-component">
