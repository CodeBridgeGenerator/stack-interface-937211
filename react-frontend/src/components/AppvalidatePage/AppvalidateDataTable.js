...
const AppvalidateDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.AppName?.appName}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.appValidation}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "appvalidate"});
...
<Column field="AppName" header="AppName" body={dropdownTemplate0} filter={selectedFilterFields.includes("AppName")} hidden={selectedHideFields?.includes("AppName")}  style={{ minWidth: "8rem" }} />
<Column field="appValidation" header="AppValidation" body={pTemplate1} filter={selectedFilterFields.includes("appValidation")} hidden={selectedHideFields?.includes("appValidation")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Appvalidate Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Appvalidate" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AppvalidateDataTable;