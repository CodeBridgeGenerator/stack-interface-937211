...
import AppvalidateDatatable from "./AppvalidateDataTable";
...
import AppvalidateFakerDialogComponent from "./AppvalidateFakerDialogComponent";
import AppvalidateSeederDialogComponent from "./AppvalidateSeederDialogComponent";

const AppvalidatePage = (props) => {
    ...
    const filename = "appvalidate.csv";
    ...
    useEffect(() => {
        ...
        client
            .service("appvalidate")
            .find({ query: { $limit: 10000 , AppName : urlParams.singleStackNameId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
    ...
    const service = _.find(config.services, { serviceName : "appvalidate"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("appvalidate").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Appvalidate", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("appvalidate").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Appvalidate </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="appvalidate-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="appvalidate-datatable">
                    <AppvalidateDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <AppvalidateEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AppvalidateCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <AppvalidateFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AppvalidateSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AppvalidatePage);
