...
const AppvalidateCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            AppName: _entity?.AppName?._id,
appValidation: _entity?.appValidation,
        };

        setLoading(true);
        try {
            
        await client.service("appvalidate").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info appvalidate updated successfully" });
    ...
    return (
        <Dialog header="Edit Appvalidate" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="appvalidate-edit-dialog-component">
