...
import seederData from "./appvalidate.seeder.json";
...
const AppvalidateSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("appvalidate").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AppvalidateSeederDialogComponent);
