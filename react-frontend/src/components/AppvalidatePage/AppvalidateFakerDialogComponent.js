...
import appvalidateFakerFactory from "./appvalidateFakerFactory";
...
const AppvalidateFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [AppNameIds, setAppNameIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("stackName")
                        ...
                        .then((data) => {
                        setAppNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = appvalidateFakerFactory(props.user,fakerCount,AppNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("appvalidate").create(elem));
...
export default connect(mapState, mapDispatch)(AppvalidateFakerDialogComponent);
