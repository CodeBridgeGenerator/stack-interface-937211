...
const StackschemaDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.rootPath}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "stackschema"});
...
<Column field="rootPath" header="RootPath" body={pTemplate0} filter={selectedFilterFields.includes("rootPath")} hidden={selectedHideFields?.includes("rootPath")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Stackschema Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Stackschema" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default StackschemaDataTable;