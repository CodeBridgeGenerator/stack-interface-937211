...
import StackschemaDatatable from "./StackschemaDataTable";
...
import StackschemaFakerDialogComponent from "./StackschemaFakerDialogComponent";
import StackschemaSeederDialogComponent from "./StackschemaSeederDialogComponent";

const StackschemaPage = (props) => {
    ...
    const filename = "stackschema.csv";
    ...
    const service = _.find(config.services, { serviceName : "stackschema"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("stackschema").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Stackschema", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("stackschema").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Stackschema </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="stackschema-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="stackschema-datatable">
                    <StackschemaDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <StackschemaEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <StackschemaCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <StackschemaFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <StackschemaSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(StackschemaPage);
