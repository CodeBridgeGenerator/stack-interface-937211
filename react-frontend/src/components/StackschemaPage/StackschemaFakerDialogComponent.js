...
import stackschemaFakerFactory from "./stackschemaFakerFactory";
...
const StackschemaFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stackschemaFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stackschema").create(elem));
...
export default connect(mapState, mapDispatch)(StackschemaFakerDialogComponent);
