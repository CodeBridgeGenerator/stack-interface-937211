...
const SchemaDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.stack?.rootPath}</p>
const dropdownArrayTemplate2 = (rowData, { rowIndex }) => <p >{rowData.database}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "schema"});
...
<Column field="stack" header="Stack" body={dropdownTemplate1} filter={selectedFilterFields.includes("stack")} hidden={selectedHideFields?.includes("stack")}  style={{ minWidth: "8rem" }} />
<Column field="database" header="Database" body={dropdownArrayTemplate2} filter={selectedFilterFields.includes("database")} hidden={selectedHideFields?.includes("database")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Schema Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Schema" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
