...
import SchemaEditDialogComponent from "./SchemaEditDialogComponent";
import SchemaCreateDialogComponent from "./SchemaCreateDialogComponent";
import SchemaFakerDialogComponent from "./SchemaFakerDialogComponent";
import SchemaSeederDialogComponent from "./SchemaSeederDialogComponent";
...
const SchemaPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("schema")
            .find({ query: { $limit: 10000 , stack : urlParams.singleStackschemaId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
    ...
    const service = _.find(config.services, { serviceName : "schema"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("schema").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Schema", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("schema").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Schema </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="schema-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="schema-datatable">
                    <SchemaDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <SchemaEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SchemaCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <SchemaFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SchemaSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(SchemaPage);
