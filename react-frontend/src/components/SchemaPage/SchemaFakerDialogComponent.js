...
import schemaFakerFactory from "./schemaFakerFactory";
...
const SchemaFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("stackschema")
                        ...
                        .then((data) => {
                        setstackIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = schemaFakerFactory(props.user,fakerCount,stackIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("schema").create(elem));
...
export default connect(mapState, mapDispatch)(SchemaFakerDialogComponent);
