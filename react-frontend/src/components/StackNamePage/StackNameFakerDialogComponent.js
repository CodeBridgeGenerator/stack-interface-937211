...
import stackNameFakerFactory from "./stackNameFakerFactory";
...
const StackNameFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stackNameFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stackName").create(elem));
...
export default connect(mapState, mapDispatch)(StackNameFakerDialogComponent);
