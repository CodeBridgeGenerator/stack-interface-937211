...
const StackNameDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.appName}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "stackName"});
...
<Column field="appName" header="AppName" body={pTemplate1} filter={selectedFilterFields.includes("appName")} hidden={selectedHideFields?.includes("appName")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload StackName Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search StackName" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default StackNameDataTable;